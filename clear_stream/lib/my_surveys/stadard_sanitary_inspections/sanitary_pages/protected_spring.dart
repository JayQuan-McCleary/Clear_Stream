...
class ProtectedSpringInspection extends StatefulWidget {
  const ProtectedSpringInspection({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ProtectedSpringInspectionState createState() =>
      _ProtectedSpringInspectionState();
}

class _ProtectedSpringInspectionState
    extends State<ProtectedSpringInspection> {
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildQuestion(
                  '1. Is the spring source unprotected?'),
              const SizedBox(height: 10),
              _buildQuestion('2. Is the masonry protecting the spring faulty?'),
              const SizedBox(height: 10),
              _buildQuestion(
                  '3. Is the backfill area behind the retaining wall eroded?'),
              const SizedBox(height: 10),
              _buildQuestion(
                  '4. Does Split water flood the Collection Area?'),
              const SizedBox(height: 10),
              _buildQuestion(
                  '5. Is the fence absent or faulty?'),
              const SizedBox(height: 10),
              _buildQuestion('6. Can animals have access within ten meters of the spring?'),
              const SizedBox(height: 10),
              _buildQuestion(
                  '7. Is there a latrine uphill or within 30 meters of the spring?'),
              const SizedBox(height: 10),
              _buildQuestion(
                  '8. Does surface water collect uphill of the spring?'),
              const SizedBox(height: 10),
              _buildQuestion('9. Is the Diversion Ditch above the spring absent or non-functional?'),
              const SizedBox(height: 10),
              _buildQuestion(
                  '10. Are there any sources of pollution uphill of the spring?'),
              const SizedBox(height: 10),

              const Text(
                'Comments on Protected Spring Sanitary Inspection',
